...
let value_to_channel ?minify oc json = value_to_dst ?minify (`Channel oc) json
...
let parse_error t fmt =
  Printf.kprintf (fun msg -> raise (Parse_error (t, msg))) fmt
...
let read_error_description : [< read_error ] -> string = function
  | `Error (_loc, err) -> Format.asprintf "%a" Jsonm.pp_error err
...
(* unit *)
let unit () = `Null
let get_unit = function `Null -> () | j -> parse_error j "Ezjsonm.get_unit"

(* bool *)
let bool b = `Bool b
let get_bool = function `Bool b -> b | j -> parse_error j "Ezjsonm.get_bool"
...
(* dict *)
let dict d = `O d
let get_dict = function `O d -> d | j -> parse_error j "Ezjsonm.get_dict"
...
let t_of_sexp s =
  ...
  | _ ->
      failwith
        "Ezjsonm: t_of_sexp encountered a value (fragment) rather than a t"
