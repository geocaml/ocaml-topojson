...
let rec r_mcomment closing k d =
  ...
  else if d.c = u_times then readc (r_mcomment true k) d
  else (
    badd d;
    readc (r_mcomment false k) d)
...
let r_comment k d =
  (* comment, / was eaten. *)
  if d.c = u_slash then readc (r_scomment k) d
  else if d.c = u_times then readc (r_mcomment false k) d
  else (
    epos d;
...
let rec r_u_escape hi u count k d =
  ...
  if count > 0 then (
    ...
    | Some hi ->
        ...
        if u < 0xDC00 || u > 0xDFFF then error (err_not_lo u) k d
        else
    ...
    | None ->
        if u < 0xD800 || u > 0xDFFF then (
          baddc d u;
          k d)
        else if u > 0xDBFF then error (err_lone_lo u) k d
        else if d.c <> u_bslash then error (err_lone_hi u) k d
        else
          readc
            (fun d ->
              if d.c <> 0x75 (* u *) then error (err_lone_hi u) (r_escape k) d
              else readc (r_u_escape (Some u) 0 4 k) d)
            d
...
let rec r_string k d =
  ...
  if d.c = ux_eoi then (
    ...
    epos d;
    readc k d)
  else if d.c = u_bslash then readc (r_escape (r_string k)) d
  else (
    epos d;
    baddc d u_rep;
...
let rec r_float k d =
  ...
  if (not (is_val_sep d.c)) && d.c <> ux_eoi then (
    ...
    with Failure _ -> ret (err_number s) k d
...
let rec discard_to c1 c2 err k d =
  if d.c = c1 || d.c = c2 || d.c = ux_eoi then ret err k d
  else (
    epos d;
...
let r_arr_val k d =
  ...
  else if last_start then r_value (nxval err_exp_arr_fst) k d
  else if d.c = u_comma then readc (r_white (r_value (nxval err_exp_value) k)) d
...
let r_obj_name k d =
  ...
  let r_name err k d =
    ...
    if d.c <> u_quot then nxmem err k d
    else (
      spos d;
  ...
  else if last_start then r_name err_exp_obj_fst k d
  else if d.c = u_comma then readc (r_white (r_name err_exp_name k)) d
  else nxmem err_exp_obj_nxt k d

let r_end k d =
  (* end of input *)
  if d.c = ux_eoi then ret `End ret_eoi d
  else
    let drain k d =
      spos d;
...
let rec r_lexeme d =
  ...
  | `Os _ :: _ ->
      if d.next_name then r_white (r_obj_name r_lexeme) d
      else r_white (r_obj_value r_lexeme) d
  | [] -> r_white (r_end r_lexeme) d

let rec discard_to_white err k d =
  if is_white d.c || d.c = ux_eoi then ret err k d
  else (
    epos d;
...
let rec writeb b k e =
  ...
  if e.o_pos > e.o_max then flush (writeb b k) e
  else (
...
let rec w_json_string s k e =
  ...
  let rec loop s j pos max k e =
    if pos > max then if j > max then k e else writes s j (pos - j) k e
    else
      let next = pos + 1 in
      let escape esc =
...
let w_lexeme k e l =
  ...
  | `Os -> (
      if (* inside object. *)
         not e.next_name then w_value ~in_obj:true l k e
      else
        match l with
        | `Name n ->
            let name n k e =
              if e.minify then w_name n k e
              else writeb u_nl (w_indent (w_name n (writeb u_sp k))) e
            in
            if e.last_start then name n k e else writeb u_comma (name n k) e
        | `Oe ->
  ...
  | `As -> (
      ...
      | l ->
          let value l k e =
            if e.minify then w_value ~in_obj:false l k e
            else writeb u_nl (w_indent (w_value ~in_obj:false l k)) e
          in
          if e.last_start then value l k e else writeb u_comma (value l k) e)

let rec encode_ k e = function
  | `Lexeme l -> if e.stack = [] then expect_end l else w_lexeme k e l
  | `End as v ->
